(function(a){
    ...
    function init(){
        ...
        for(var i=1;i<=b;i++){
            a.append('<li style="background-image:url('+d+'/star-rating.png)"><div></div></li>')
            }
            a.insertAfter(q);
        q.next("ul").children("li").children("div").css('background-image','url('+d+'/star-rating.png)');
...
$("#rating_simple2").webwidget_rating_bar({
				...
				$("#rating_simple3").webwidget_rating_bar({
                    rating_star_length: '5',
                    rating_default_value: '0',
                    rating_function_name: '',//this is function name for click
                    directory: 'images/'
